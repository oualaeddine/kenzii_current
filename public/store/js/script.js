...
$(document).on("ready", function () {
    ...
    $(".lx-hero-item").each(function () {
        $(this).css({
            "background": "url(" + $(this).attr("data-bg") + ") no-repeat",
            "background-size": "cover",
            "height": ($(this).width() * 0.7) + "px"
        });
    });
...
$(".lx-purchase-btns-floating a").on("click", function () {
    $(".lx-add-to-cart").html('<i class="fas fa-circle-notch fa-spin"></i> الرجاء الانتظار');
...
$(".lx-cart-next-step a").on("click", function () {
    if (!clicked) {
        ...
        if (save === "yes") {
            ...
            $(".lx-floating-response").remove();
            window.clearTimeout(timer);
            $("body").append('<div class="lx-floating-response"><p class="lx-error"><i class="material-icons">error_outline</i> الرجاء ملأ الاستمارة بالكامل<i class="material-icons">close</i></p></div>');
            $(".lx-floating-response").fadeIn();
            timer = window.setTimeout(function () {
                $(".lx-floating-response").fadeOut();
            }, 5000);
