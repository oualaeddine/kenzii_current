...
<div class="horizontal-menu-wrapper">
    <div class="header-navbar navbar-expand-sm navbar navbar-horizontal {{$configData['horizontalMenuClass']}} {{($configData['theme'] === 'dark') ? 'navbar-dark' : 'navbar-light' }} navbar-shadow menu-border"
                    ...
                    @foreach($menuData[1]->menu as $menu)
                        ...
                        @if(isset($menu->submenu)){{'data-menu=dropdown'}}@endif>
                            <a href="{{isset($menu->url)? url($menu->url):'javascript:void(0)'}}"
                               ...
                               target="{{isset($menu->newTab) ? '_blank':'_self'}}" @if(isset($menu->submenu)){{'data-toggle=dropdown'}}@endif>
