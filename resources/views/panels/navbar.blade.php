    ...
    <nav class="header-navbar navbar-expand-lg navbar navbar-fixed align-items-center navbar-shadow navbar-brand-center {{ $configData['navbarColor'] }}"
        ...
        @else
            <nav class="header-navbar navbar navbar-expand-lg align-items-center {{ $configData['navbarClass'] }} navbar-light navbar-shadow {{ $configData['navbarColor'] }}">
                ...
                <div class="navbar-container d-flex content">
                        ...
                        <ul class="nav navbar-nav">
                            ...
                            <li class="nav-item nav-search"><a class="nav-link nav-link-search">
                                ...
                                <div class="search-input">
                                    ...
                                    <input class="form-control input" type="text" placeholder="Search order..."
                    ...
                    <ul class="nav navbar-nav align-items-center ml-auto">
                        ...
                        <li class="nav-item">
                            <a href="/download_apk" class="nav-link">
                                <i class="fa fa-mobile-alt"></i>
                            </a>
                        </li>

                        <li class="nav-item d-lg-block">
                            <a class="nav-link " href="javascript:;" onclick="SmsModal()">
                            ...
                            <a class="nav-link dropdown-toggle dropdown-user-link" id="dropdown-user"
                               ...
                               aria-expanded="false">
                                <div class="user-nav d-sm-flex d-none">
                                    ...
                                    <span class="user-status">{{\Illuminate\Support\Facades\Auth::user()->roles->first()->name}}</span>
                                </div>
                                <span class="avatar">
              <img class="round" src="{{asset('images/portrait/small/avatar-s-2.jpg')}}" alt="avatar" height="40"
                   width="40">
                 {{-- todo add logged in user idle status--}}
            ...
            @php
                ...
                $array = collect(Modules\Orders\Entities\Order::select('name','id','last_status as status')->get());

                $array_two = collect(Modules\Orders\Entities\Order::select('phone as name','id','last_status as status')->get());

                $array_three = collect(Modules\Orders\Entities\Order::select('id as name','id','last_status as status')->get());

                $array_final =  $array->merge($array_two);

                $array_final =  $array_final->merge($array_three);
